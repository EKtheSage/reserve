...
cycle_index <- data.table(
  accident_year = sort(unique(model_data$accident_year)),
  RLM = c(1, 1.18, 1.22, 1.05, 1, 0.87, 0.94, 1.34, 1.64, 2.14)^0.6,
  RRM = c(1, 1.05, 1.05, 1.01, 1.0, 0.95, 0.99, 1.1, 1.25, 1.35)^0.6
)

setkey(model_data, accident_year)
setkey(cycle_index, accident_year)
model_data <- cycle_index[model_data]
...
fit <- brm(
  ...
  backend = 'cmdstan',
  file = 'hierarchical-compartmental-pricing-reserving-cycle',
